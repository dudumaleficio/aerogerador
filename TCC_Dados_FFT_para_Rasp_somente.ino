...
double fftResult[samples];
double peak;

arduinoFFT FFT;


//_________________Variaveis utilizadas para o BME280_________________//
...
const char* serverName = "http://10.0.0.220/post-esp-data.php";  //PHP BD
...
void sensorCorrente()  //Rotina para a interrupção do anemometro
{
  ...
  if ((millis() - tempo2) > amostragem) {
    ...
    for (uint16_t i = 0; i < 32; i++) {
      if (WiFi.status() == WL_CONNECTED) {  //Se o wifi estiver conectado
        ...
        Serial.println(httpRequestFFTData);
